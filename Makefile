...
function is_pip_ver_available() {
    local ver="$$0"
    local out="$$(pip install ipygpulogger== |& grep $$ver)"
    if [[ -n "$$out" ]]; then
        echo 1
    else
        echo 0
...
function wait_till_pip_ver_is_available() {
    ...
    if [[ $$(is_pip_ver_available $$ver) == "1" ]]; then
        echo "ipygpulogger-$$ver is available on pypi"
        return 0
    fi

    COUNTER=0
    echo "waiting for ipygpulogger-$$ver package to become visible on pypi:"
    ...
    echo -e "ipygpulogger-$$ver is now available on pypi"
}
...
test-install: ## test conda/pip package by installing that version them
	...
	@pip uninstall -y ipygpulogger

	@echo "\n\n*** waiting for $(version) pip version to become visible"
	bash -c "$$WAIT_TILL_PIP_VER_IS_AVAILABLE_BASH" $(version)

	pip install ipygpulogger==$(version)
	pip uninstall -y ipygpulogger

	@echo "\n\n*** Install/uninstall $(version) conda version"
	@# skip, throws error when uninstalled @conda uninstall -y ipygpulogger

	@echo "\n\n*** waiting for $(version) conda version to become visible"
	@perl -e '$$v=shift; $$p="ipygpulogger"; $$|++; sub ok {`conda search -c stason $$p==$$v >/dev/null 2>&1`; return $$? ? 0 : 1}; print "waiting for $$p-$$v to become available on conda\n"; $$c=0; while (not ok()) { print "\rwaiting: $$c secs"; $$c+=5;sleep 5; }; print "\n$$p-$$v is now available on conda\n"' $(version)

	conda install -y -c fastai -c stason ipygpulogger==$(version)
	@# leave conda package installed: conda uninstall -y ipygpulogger
...
# 	coverage run --source ipygpulogger -m pytest
# 	coverage report -m
# 	coverage html
